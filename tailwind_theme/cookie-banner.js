...
(function () {
  ...
  function saveConsent(analytics, marketing, action) {
    var record = {
  ...
  /* Apply stored consent after other scripts can listen for pwi:consent */
  (function applyStoredConsentEarly() {
    var existing = getConsent();
    if (existing && existing.version === POLICY_VERSION) {
      setTimeout(function () { applyConsent(existing); }, 0);
    }
  })();
  ...
  function wireModal(overlay, banner) {
    document.getElementById('pwi-pref-reject-all').addEventListener('click', function () {
      ...
      saveConsent(false, false, 'reject-all');
      closeModal(overlay);
  ...
  function init() {
    ...
    document.getElementById('pwi-cb-close').addEventListener('click', function () {
      saveConsent(false, false, 'dismissed');
      hideBanner(built);
    });
